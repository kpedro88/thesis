...
The superpotential is limited to these terms by requiring the conservation of $R$-parity. The quantity $R$ is related to the baryon and lepton numbers as well as the particle spin $S$ and may be defined in two equivalent ways, as shown in Eq. \eqref{eq:Rdef} \cite{Barbier}. Equation \eqref{eq:Rparity} defines $R$-parity based on $R$.
\begin{align}
...
In R-parity-conserving (RPC) SUSY, all decays of superpartners eventually produce at least one LSP. As a weakly interacting particle, the LSP will escape a particle detector without being detected, leading to events with signatures including large missing transverse momentum or \MET. 
%talk about limits on RPC/large MET searches, other models besides MSSM

%introduce RPV - no DM, still solves hierarchy, superpotential, need for single type of coupling "turned on" to avoid proton decay

%lambda'333 decay - same production (within 2%, cite) and final-state kinematics

%lambda'321 decay - model details

%top squark search terminology - B, etc.