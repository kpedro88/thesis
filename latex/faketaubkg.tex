...
% \subsubsection{Fake tau background from W, Z and ttbar processes in etau channel}
% \label{sec:faketaubkgetau}

% In the \etau channel, there is a significant contribution from QCD events in the anti-isolated region. As described previously at the beginning of Sec.~\ref{subsec:faketaubkg}, the fake rate calculated from the \Zmm control region is not appropriate for QCD events. The QCD contribution must be subtracted from the fake tau background estimation performed with the fake rate method. The same sign opposite sign (SS/OS) method is used to estimate the number of QCD events in the anti-isolated region. 
...
% Tables \ref{Bkg:tab:faketauresultsetauLQ} and \ref{Bkg:tab:faketauresultsetauLQD} show the results of the fake tau background estimation for the different final selections in the \etau channel, with the QCD contribution already subtracted. The sources of systematic uncertainty are the same as those considered in the \mutau channel (Sec. \ref{subsubsec:faketaubkgmutau}), with the uncertainty on the subtracted QCD contribution included as an additional systematic uncertainty. The results from the fake rate method are consistent with the MC predictions. The total uncertainty on the fake tau background yield is $17\%$ for the LQ final selection and $24\%$ for the LQD321 final selection.
...
%       \multicolumn{2}{|c|}{Sources} & \multicolumn{2}{|c|}{LQ Results} \\
%       \hline
%       anti-iso    & fake rate                                & yield  & variation \\
%       \hline
%       data        & data (incl.)                             & 124.2 & (central)   \\
%       data        & data (incl.) $\pm 1\sigma_{\text{stat}}$ & 138.5 $\text{*}$ & 14.3 \\
%       data        & data ($N_{\text{jet}} \geq 1$)           & 113.9 & 10.3         \\ 
%       residual MC & data (incl.)                             & ---   & 7.1              \\
%       QCD         & data (incl.)                             & ---   & 2.7 $\text{**}$  \\
%       \multicolumn{2}{|c|}{\Zmm vs. \ttbar fake rates ($18\%\times35\%$)} & --- & 7.8 \\
%       \hline
%       \multicolumn{2}{|c|}{final result} & \multicolumn{2}{|c|}{124.2 $\pm$ 20.7} \\
%       \multicolumn{2}{|c|}{MC prediction} & \multicolumn{2}{|c|}{106.8 $\pm$ 27.3} \\
%       \multicolumn{2}{|c|}{MC closure} & \multicolumn{2}{|c|}{129.5 $\pm$ 3.6} \\
%       \hline
%     \end{tabular}
%     \caption{Results of the data-driven fake tau background estimation for the LQ final selection in the \etau channel. 
% This table shows all sources of systematic uncertainty and a comparison to the generator-level MC prediction. 
% Only statistical uncertainty is given for the MC prediction and the MC closure test. $\text{*}$ From $\text{FR}+1\sigma_{\text{stat}}$ and $\text{FR}-1\sigma_{\text{stat}}$, the larger variation from the central value is used. $\text{**}$ The QCD line corresponds to the systematic uncertainty assigned to the QCD subtraction from the fake tau background.}
...
%       \multicolumn{4}{c}{\etau channel} \\
%       \hline
%       \multicolumn{2}{|c|}{Sources} & \multicolumn{2}{|c|}{LQD321 Results} \\
%       \hline
%       anti-iso    & fake rate                                                  & yield  & variation\\
%       \hline
%       data        & data ($N_{\text{jet}} \geq 1$)                             & 65.7 & (central) \\
%       data        & data ($N_{\text{jet}} \geq 1$) $\pm 1\sigma_{\text{stat}}$ & 74.6 $\text{*}$ & 8.9 \\
%       data        & data ($N_{\text{jet}} \geq 2$)                             & 59.8 & 5.9   \\
%       residual MC & data ($N_{\text{jet}} \geq 1$)                             & ---  & 1.8      \\
%       QCD         & data ($N_{\text{jet}} \geq 1$)                             & ---  & 0.1 $\text{**}$ \\
%       \multicolumn{2}{|c|}{\Zmm vs. \ttbar fake rates ($18\%\times95\%$)}      & ---  & 11.23 \\
%       \hline
%       \multicolumn{2}{|c|}{final result} & \multicolumn{2}{|c|}{65.7 $\pm$ 15.6}\\
%       \multicolumn{2}{|c|}{MC prediction} & \multicolumn{2}{|c|}{66.7 $\pm$ 7.6} \\
%       \multicolumn{2}{|c|}{MC closure} & \multicolumn{2}{|c|}{61.6 $\pm$ 1.4} \\
%       \hline
%     \end{tabular}
%     \caption{Results of the data-driven fake tau background estimation for the LQD321 final selection in the \etau channel. 
% This table shows all sources of systematic uncertainty and a comparison to the generator-level MC prediction. 
% Only statistical uncertainty is given for the MC prediction and MC closure test. $\text{*}$ From $\text{FR}+1\sigma_{\text{stat}}$ and $\text{FR}-1\sigma_{\text{stat}}$, the larger variation from the central value is used. $\text{**}$ The QCD line corresponds to the systematic uncertainty assigned to the QCD subtraction from the fake tau background.}
